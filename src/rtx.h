...
int* rtx_rmq_idx(int alg, int n, int bs, int q, float *darray, int2 *dquery, CmdArgs args) {
    int dev = args.dev;
    int reps = args.reps;
    Timer timer;
    int *output, *d_output;
    //float cpuMin=-1.0f;
    output = (int*)malloc(q*sizeof(int));
    ...
    // 4) Populate and move parameters to device (ONCE)
    printf("device params to GPU "); fflush(stdout);
    CUDA_CHECK( cudaMalloc(&d_output, q*sizeof(int)) );
    timer.restart();
    Params params;
    Params *device_params;
    ...
    if (args.save_power)
        GPUPowerEnd();
    float timems = timer.get_elapsed_ms();
    CUDA_CHECK( cudaMemcpy(output, d_output, q*sizeof(int), cudaMemcpyDeviceToHost) );
    float avg_time = timems/(1000.0*reps);
    printf(AC_BOLDCYAN "done: %f secs (avg %f secs): [%.2f RMQs/sec, %f nsec/RMQ]\n" AC_RESET, timems/1000.0, avg_time, (double)q/avg_time, (double)avg_time*1e9/q);
    write_results(timems, q, geom_time + AS_time, reps, args, mem);
