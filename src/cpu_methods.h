...
// CPU min idx
template <typename T>
int cpu_min_idx(T *A, int l, int r) {
    T min = A[l];
    int min_idx = l;
    for (int i = l; i <= r; ++i) {
        if (A[i] < min){
...
template <typename T>
int* cpu_rmq_idx(int n, int nq, T *A, int2 *Q, int nt, CmdArgs args) {
    int reps = args.reps;
    Timer timer;
    int* out = new int[nq];

    omp_set_num_threads(nt);
    printf(AC_BOLDCYAN "Computing RMQs index (%-11s,nt=%2i,r=%-3i).." AC_RESET, algStr[ALG_CPU_BASE], nt, reps); fflush(stdout);
    timer.restart();
    for (int i = 0; i < reps; ++i) {
        #pragma omp parallel for shared(out, A, Q)
        for (int i = 0; i < nq; ++i) {
            out[i] = cpu_min_idx<T>(A, Q[i].x, Q[i].y);
        }
    }
    printf("dbg5\n"); fflush(stdout);
    timer.stop();
...
int *rmq_rmm_par_idx(int n, int nq, int *A, int2 *Q, int nt, CmdArgs args) {
    using namespace rmqrmm;
    omp_set_num_threads(nt);
    ...
    //printf("%sAnswering Querys [%2i threads]......", AC_BOLDCYAN, nt); fflush(stdout);
    printf(AC_BOLDCYAN "Computing RMQs index (%-11s,nt=%2i,r=%-3i).." AC_RESET, algStr[ALG_CPU_HRMQ], nt, reps); fflush(stdout);
    if (args.save_power)
        //CPUPowerBegin("HRMQ", 100, args.time_file);
        CPUPowerBegin("HRMQ", 500, args.power_file);
    timer.restart();
