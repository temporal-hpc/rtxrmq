...
extern "C" __global__ void __raygen__rmq_blocks_idx() {
  ...
  unsigned int missSBTindex = 0;
  unsigned int payload = __float_as_uint(max);
  unsigned int min_idx = __float_as_uint(max);
  ...
  if (lB == rB) {
    ...
    ray_direction = make_float3(1.0, 0.0, 0.0);
    optixTrace(params.handle, ray_origin, ray_direction, tmin, tmax, ray_time,
        visibilityMask, rayFlags, SBToffset, SBTstride, missSBTindex, payload, min_idx);
#ifdef DBG_RAY
    if (idx.x == DBG_RAY)
      printf("1B Ray %i, query (%i,%i)\n    bx %i    by %i   mx %i   my %i \n    lB = %i,  rB = %i  nb = %i\n    x = %.10f,  y = %.10f\n    payload: %f  min_idx: %i\n", idx.x, q.x, q.y, bx, by, mx, my, lB, rB, nb_offset, x, y, __uint_as_float(payload), min_idx);
#endif

    params.idx_output[idx.x] = min_idx - nb_offset;
    return;
  }
  ...
  if (lB < rB-1) {
    ...
    optixTrace(params.handle, ray_origin, ray_direction, tmin, tmax, ray_time,
        visibilityMask, rayFlags, SBToffset, SBTstride, missSBTindex, payload, min_idx);
    min_idx = params.min_block[min_idx] + nb_offset;
#ifdef DBG_RAY
    if (idx.x == DBG_RAY)
      printf("\nray %i,  (l,r)=(%i, %i)\n    bs = %i    nb = %i\n    x = %.15f,  y = %.15f\n    payload %f\n", idx.x, q.x, q.y, block_size, num_blocks, x, y, __uint_as_float(payload));
#endif
...
extern "C" __global__ void  __closesthit__rmq_blocks_idx() {
  float curr_tmax = optixGetRayTmax();
#ifdef DBG_RAY
  uint3 idx = optixGetLaunchIndex();
  int Pidx = optixGetPrimitiveIndex();
...
#endif
  float prev_val = __uint_as_float(optixGetPayload_0());
  if (curr_tmax < prev_val) {
    optixSetPayload_0(__float_as_uint(curr_tmax));
    int idx = optixGetPrimitiveIndex();
    optixSetPayload_1(idx);
  }
}
